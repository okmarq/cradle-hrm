...
class LanguageDao extends BaseDao
{
    ...
    {
        try {
            $language->setOrganizationId($this->auth->getOrganizationId());
    ...
    public function getLanguageByName(string $name): ?Language
    {
        try {
            ...
            $query->where('l.organizationId = :organizationId')
                ->setParameter('organizationId', $this->auth->getOrganizationId());
            $trimmed = trim($name, ' ');
    ...
    public function getLanguageListPaginator(
        ...
        $q->where('l.organizationId = :organizationId')
                ->setParameter('organizationId', $this->auth->getOrganizationId());
    ...
    public function isExistingLanguageName(string $languageName): bool
    {
        try {
            ...
            $q->andWhere('l.organizationId = :organizationId')
                ->setParameter('organizationId', $this->auth->getOrganizationId());
            $count = $this->count($q);
            if ($count > 0) {
                return true;
