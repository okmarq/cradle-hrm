...
class SignUpController extends AbstractVueController implements PublicControllerInterface
{
    use AuthUserTrait;
    ...
    {
        $component = new Component('auth-signup');
        if ($this->getAuthUser()->hasFlash(AuthUser::FLASH_SIGNUP_ERROR)) {
            $error = $this->getAuthUser()->getFlash(AuthUser::FLASH_SIGNUP_ERROR);
            $component->addProp(
                new Prop(
                    'error',
                    Prop::TYPE_OBJECT,
        ...
        $component->addProp(
            new Prop(
                'token',
                Prop::TYPE_STRING,
                $this->getCsrfTokenManager()->getToken('signup')->getValue()
            )
        );
        $component->addProp(
