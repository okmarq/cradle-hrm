...
class ResetPasswordController extends AbstractController implements PublicControllerInterface
{
    ...
    public function handle(Request $request): RedirectResponse
    {
        ...
        $resetCode = $request->request->get('_reset');

        if (!$this->getCsrfTokenManager()->isValid('reset-user-password', $token)) {
            throw AuthenticationException::invalidCsrfToken();
        }

        // $username = $request->request->get('username');

        $username =  $this->getResetPasswordService()->retrieveResetUser($resetCode);
        ...
        return $this->redirect("auth/resetUserSuccess");
    }
}
