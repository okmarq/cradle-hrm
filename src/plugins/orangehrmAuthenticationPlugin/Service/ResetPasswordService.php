...
class ResetPasswordService
{
    ...
    public function generateResetCode($length = 20): string
    {
        $characters =
    ...
    public function storeResetUser(
        string $username,
        string $resetCode
    ) {
        try {
            ...
            ->createQueryBuilder()
            ->delete('ohrm_resetPassword')
            ->where('ohrm_resetPassword.user = :user')
            ->setParameter('user', $username)
            ->executeQuery();
    
        $result = Connection::getConnection()
            ->createQueryBuilder()
            ->insert('ohrm_resetPassword')
            ->values([
                'user' => ':user',
                'code' => ':code',
            ])
            ->setParameter('user', $username)
            ->setParameter('code', $resetCode)
            ->executeQuery();
    ...
    public function retrieveResetUser(string $resetCode)
    {
        return Connection::getConnection()
            ->createQueryBuilder()
            ->select('userMail.user')
            ->from('ohrm_resetPassword', 'userMail')
            ->where('userMail.code = :code')
            ->setParameter('code', $resetCode)
            ->setMaxResults(1)
            ->fetchOne();
    }
    ...
    public function searchForUsername(string $username): ?User{
        return $this->getUserService()->searchUsername($username);
    }
    /**
    ...
    public function sendPasswordResetCodeEmail(Employee $receiver, string $resetCode, string $userName): bool
    {
        try {
            ...
            $this->getEmailService()->setMessageFrom(
                [$this->getEmailService()->getEmailConfig()->getSentAs() => 'CradleHRM']
            );
            $this->getEmailService()->setMessageSubject('CradleHRM Password Reset');
