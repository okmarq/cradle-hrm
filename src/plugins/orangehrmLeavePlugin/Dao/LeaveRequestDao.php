...
class LeaveRequestDao extends BaseDao
{
    ...
    public function saveLeaveRequest(
        ...
        try {
            ...
            $this->triggerEmailEvent($leaveRequest);
            return $leaveRequest;
        } catch (Exception $e) {
    ...
    public function triggerEmailEvent(LeaveRequest $leaveRequest)
    {

        $employeeDao = new EmployeeDao();
        $employee = $employeeDao->getEmployeeByEmpNumber($leaveRequest->getEmployee()->getEmpNumber());
        ...
        $dateApplied = $leaveRequest->getDateApplied()->format('Y-m-d');
        $leaveDetails = $leaveRequest->getLeaves();
        $numberOfHours = intVal($leaveDetails[0]->getLengthHours());
        $numberOfDays = intVal($leaveDetails[0]->getLengthDays());

        $subject = <<<subject
        Leave Notification - $name Applied for $numberOfDays Day(s) of $leaveType Leave
        subject;
        ...
        $subscriber = new LeaveEventSubscriber();
        $dispatcher->addSubscriber($subscriber);
        $event = new LeaveApplyEvent($this->supervisors($employee), $content);
        $dispatcher->dispatch($event, LeaveApplyEvent::APPLY);
    }

    public function supervisors(Employee $performer): array
    {
        $recipients = [];
