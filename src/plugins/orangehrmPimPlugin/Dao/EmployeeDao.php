...
class EmployeeDao extends BaseDao
{
    ...
    public function getEmployeeList(
        ...
        $qb = $this->getEmployeeListQueryBuilderWrapper(
            $employeeSearchParamHolder
        )->getQueryBuilder();
     ...
     * @return int[]
     */
    public function getEmpNumbersByFilterParams(
        EmployeeSearchFilterParams $employeeSearchParamHolder
    ): array {
        $employeeSearchParamHolder->setSortField('employee.empNumber');
        $q = $this->getEmployeeListQueryBuilderWrapper(
            $employeeSearchParamHolder
        )->getQueryBuilder();
    ...
    protected function getEmployeeListQueryBuilderWrapper(
        ...
        if (
            $this->getTextHelper()->strStartsWith(
                $employeeSearchParamHolder->getSortField(),
                'supervisor'
            )
        ) {
        ...
        if (
            ...
            $q->andWhere(
                $q->expr()->isNull('employee.employeeTerminationRecord')
            );
        } elseif (
            $employeeSearchParamHolder->getIncludeEmployees() ===
            ...
            $q->andWhere(
                $q->expr()->isNotNull('employee.employeeTerminationRecord')
            );
        }
        ...
        if (!is_null($employeeSearchParamHolder->getName())) {
            ...
            $q->setParameter(
                'name',
                '%' . $employeeSearchParamHolder->getName() . '%'
            );
        }
        ...
        if (!is_null($employeeSearchParamHolder->getNameOrId())) {
            ...
            $q->setParameter(
                'nameOrId',
                '%' . $employeeSearchParamHolder->getNameOrId() . '%'
            );
        }

        if (!is_null($employeeSearchParamHolder->getSubunitId())) {
            $q
                ->andWhere($q->expr()->in('subunit.id', ':subunitIds'))
                ->setParameter(
                    'subunitIds',
                    $employeeSearchParamHolder->getSubunitIdChain()
                );
        }
        ...
        if (!is_null($employeeSearchParamHolder->getLocationId())) {
            $q
                ...
                ->setParameter(
                    'locationId',
                    $employeeSearchParamHolder->getLocationId()
                );
        }

        if (!is_null($employeeSearchParamHolder->getEmpStatusId())) {
            $q
                ->andWhere('empStatus.id = :empStatusId')
                ->setParameter(
                    'empStatusId',
                    $employeeSearchParamHolder->getEmpStatusId()
                );
        }

        if (!is_null($employeeSearchParamHolder->getJobTitleId())) {
            $q
                ->andWhere('jobTitle.id = :jobTitleId')
                ->setParameter(
                    'jobTitleId',
                    $employeeSearchParamHolder->getJobTitleId()
                );
        }
        ...
        if (!is_null($employeeSearchParamHolder->getEmployeeNumbers())) {
            $q
                ...
                ->setParameter(
                    'empNumbers',
                    $employeeSearchParamHolder->getEmployeeNumbers()
                );
        }
        ...
        if (!is_null($employeeSearchParamHolder->getSupervisorEmpNumbers())) {
            ...
            $q
                ->andWhere(
                    $q
                        ->expr()
                        ->in('supervisor.empNumber', ':supervisorEmpNumbers')
                )
                ->setParameter(
                    'supervisorEmpNumbers',
                    $employeeSearchParamHolder->getSupervisorEmpNumbers()
                );
        }
    ...
    public function getSubordinateIdListBySupervisorId(
        ...
        $q
            ->andWhere('r.supervisor = :supervisorId')
    ...
    public function getNumberOfEmployees(bool $includeTerminated = false): int
    {
        ...
        $q->where('e.organizationId = :organizationId')
            ->setParameter('organizationId', $auth->getOrganizationId());
    ...
    public function getReportingMethod(
        ...
        $q
            ->leftJoin('rm.reportTos', 'reportTo')
     ...
     * @return int[]
     */
    public function getEmpNumberList(
        bool $excludeTerminatedEmployees = false
    ): array {
        $q = $this->createQueryBuilder(Employee::class, 'e');
    ...
    public function getSupervisorIdListBySubordinateId(
        ...
        $q = $this->createQueryBuilder(ReportTo::class, 'r');
        $q
            ->andWhere('r.subordinate = :subordinateId')
            ->setParameter('subordinateId', $subordinateId);
     ...
     * @return Employee[]
     */
    public function getAvailableEmployeeListForWorkShift(
        EmployeeSearchFilterParams $employeeSearchParamHolder
    ): array {
        $q = $this->getEmployeeListQueryBuilderWrapper(
            $employeeSearchParamHolder
